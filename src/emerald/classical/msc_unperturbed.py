...
def MsC_momentum(alpha: float, E: float, r: float) -> float:
    ...
    return ( np.sqrt(2*(E - MsC_potential(alpha, r))) )


@njit
def MsC_diff_system(alpha: float, t: float, X: np.ndarray) -> np.ndarray:
    ...
    f1 = p
    f2 = - MsC_potential_1st_derivative(alpha, r)
...
def runge_kuta_4(alpha: float, t: float, X: np.ndarray, dt: float = 1.e-4) -> np.ndarray:
    ...
    k1 = MsC_diff_system(alpha, t, X)
    k2 = MsC_diff_system(alpha, t + (1/2)*dt, X + (1/2)*dt*k1)
    k3 = MsC_diff_system(alpha, t + (1/2)*dt, X + (1/2)*dt*k2)
    k4 = MsC_diff_system(alpha, t + dt, X + dt*k3)

    return ( X + (dt/6)*(k1 + 2*k2 + 2*k3 + k4) )


@njit 
def MsC_bound_solution( alpha: float, r0: float, E: float, tot_time: float, m: float = 1, dt: float = 1.e-4, one_cycle: bool = False ):
    ...
    p0 = MsC_momentum(alpha, E, r0)                 #momentum at the start (x0 and t0) 
...
@njit
def MsC_phase_space( alpha: float, E: float, dt: float = 1.e-4, rM: float = 0. ) -> np.ndarray:
    ...
    if E < 0:
        ...
        rm = MsC_return_points( alpha, E )[0]

    rs = np.arange( rm, rM, dt )
    ps = np.zeros_like(rs)
    ps[1:len(ps)-1] = np.array([ MsC_momentum(alpha, E, r) for r in np.arange( rm+dt, rM-dt, dt )] )
...
@njit
def MsC_action( alpha: float, E: float, dr: float = 1.e-6 ) -> float:
    ...
    for i in range(1, n):
        r = rm + i * dr
        if i % 2 == 0:
            sum2 += MsC_momentum( alpha, E, r )
        else:
            sum1 += MsC_momentum( alpha, E, r )
...
@njit
def MsC_angular_frequency( alpha: float, E: float, dE: float = 1.e-4 ) -> float:
    
    """Calculates the agular frequency of a particle in the Morse-Coulomb potential derivating action"""


    return ( (1/(12*dE))*( MsC_action(alpha, E-2*dE, dE ) 
                          - 8*MsC_action(alpha, E-dE, dE ) 
                          + 8*MsC_action(alpha, E+dE, dE ) 
                          - MsC_action(alpha, E+2*dE, dE ) 
                          ) 
            )**(-1)


@njit
def MsC_angle( alpha: float, E: float, r: float, omg_n: float, dr: float = 1.e-5 ) -> float:
    ...
    for i in range(1, n):
        ri = rm + i * dr
        if i % 2 == 0:
            sum2 += ( 2*(E - MsC_potential(alpha, ri)) )**(-1/2)
        else:
            sum1 += ( 2*(E - MsC_potential(alpha, ri)) )**(-1/2)
...
@njit
def MsC_position( angles: np.ndarray, alpha: float, Ei: float, N: int, dr: float = 1.e-5 ) -> np.ndarray:

    success = False

    while success == False:
        rmin_mc, rmax_mc = MsC_return_points(alpha, Ei)                                         #pontos de retorno
        rs_mc = np.array(sorted( chebyshev_nodes( rmin_mc, rmax_mc, N ) ))                                #distribuicao de chebyshev para evitar fenomeno de runge

        omg_n = MsC_angular_frequency(alpha, Ei, dr)

        thetas_mc = np.array( [ MsC_angle( alpha, Ei, r, omg_n, 1.e-6 ) for r in rs_mc ] )
