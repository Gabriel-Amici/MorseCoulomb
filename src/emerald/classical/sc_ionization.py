...
@njit(parallel=True)
def sC_ionization_probability(alpha: float, E_0: float, F_0: float, Omg: float, r0s: np.ndarray, p0s: np.ndarray, t_0: float, total_time: float, dt: float = 1.e-5) -> float:
...
@njit(parallel=True)
def sC_ionization_probability_criteria(alpha: float, E_0: float, F_0: float, Omg: float, r0s: np.ndarray, p0s: np.ndarray, t_0: float, total_time: float, dt: float = 1.e-5) -> float:
    ...
    for n in prange(0, Num_conditions):
        ...
        while t < total_time + t_0:
            ...
            if (sC_compensate_energy(alpha, X[0], X[1], F_0, Omg, t) > 0):
                occurences += 1
            else:
                occurences = 0
...
@njit(parallel=True)
def sC_ionization_amplitude_criteria( alpha: float, E_0: float, F_0s: np.ndarray, Omg: float, Num_trajectories: int, t_0: float, total_time: float, poly_degree: int, dt: float = 1.e-4 ):
    ...
    for f in prange(len(F_0s)):
        Pi_8rM, Pi_4rM, Pi_comp = sC_ionization_probability_criteria( alpha, E_0, F_0s[f], Omg, r0s, p0s, t_0, total_time, dt )
...
def sC_ionization_frequency_criteria( alpha: float, E_0: float, F_0: float, Omgs: np.ndarray, Num_trajectories: int, total_time: float, poly_degree: int, dt: float = 1.e-4 ):
    ...
    for o in range(len(Omgs)):
        Omg = Omgs[o]
        t_0 = np.pi/(2*Omg)
        Pi_8rM, Pi_4rM, Pi_comp = sC_ionization_probability_criteria( alpha, E_0, F_0, Omg, r0s, p0s, t_0, total_time, dt )
...
@njit #(parallel=True)
def sC_ionization_alphas( alphas: np.ndarray, E_0: float, F_0: float, Omg: float, Num_trajectories: int, t_0: float, total_time: float, poly_degree: int, dt: float = 1.e-4 ):
...
@njit #(parallel=True)
def sC_ionization_trajectories( alpha: float, E_0: float, F_0: float, Omg: float, Num_trajectories: np.ndarray, t_0: float, total_time: float, dt: float = 1.e-4 ):
